<!-- TOC -->
  - [第一部分 各比对流程入口函数分析](#第一部分-各比对流程入口函数分析)
    - [1. main函数执行逻辑](#1-main函数执行逻辑)
      - [1.1 函数目标](#1-1-函数目标)
      - [1.2 main函数整体架构流程图](#1-2-main函数整体架构流程图)
        - [1.2.1 参数解析](#1-2-1-参数解析)
        - [1.2.2 循环解析参数后全局校验](#1-2-2-循环解析参数后全局校验)
        - [1.2.3 构建输入文件列表](#1-2-3-构建输入文件列表)
        - [1.2.4 模式分发与比对流程](#1-2-4-模式分发与比对流程)
      - [1.3 函数优化点建议](#1-3-函数优化点建议)
        - [1.3.1 -mm 1与-dir/-dir1/-dir2组合优化](#1-3-1--mm-1与-dir--dir1--dir2组合优化)
        - [1.3.2 -I/-i的输入序列与pdb输入结构的校验补充](#1-3-2--i--i的输入序列与pdb输入结构的校验补充)
    - [TMalign 函数执行逻辑](#tmalign-函数执行逻辑)
      - [2.1 函数目标](#2-1-函数目标)
      - [2.2 输入与输出](#2-2-输入与输出)
      - [2.3 执行流程主线](#2-3-执行流程主线)
        - [2.3.1 文件解析](#2-3-1-文件解析)
        - [2.3.2 选择比对模式并调用核心比对函数](#2-3-2-选择比对模式并调用核心比对函数)
        - [2.3.3 结果输出](#2-3-3-结果输出)
        - [2.3.4 内存清理](#2-3-4-内存清理)
      - [2.4 核心数据流闭环](#2-4-核心数据流闭环)
      - [2.5代码执行逻辑详细流程图](#2-5代码执行逻辑详细流程图)
      - [2.6 总结](#2-6-总结)
      - [2.7 函数优化点建议](#2-7-函数优化点建议)
        - [2.7.1 get_PDB_lines函数优化](#2-7-1-get_pdb_lines函数优化)
        - [2.7.2 mmcif读取流程优化](#2-7-2-mmcif读取流程优化)
        - [2.7.3 链对比对完成后输出流程优化](#2-7-3-链对比对完成后输出流程优化)
        - [2.7.4 内存分配与释放流程优化](#2-7-4-内存分配与释放流程优化)
    - [MMalign 函数执行逻辑](#mmalign-函数执行逻辑)
      - [3.1 函数目标](#3-1-函数目标)
      - [3.2 输入与输出](#3-2-输入与输出)
      - [3.3 执行流程主线](#3-3-执行流程主线)
        - [3.3.1 解析复合物（parse_chain_list）](#3-3-1-解析复合物-parse_chain_list)
        - [3.3.2 处理用户强制链映射（-chainmap）](#3-3-2-处理用户强制链映射--chainmap)
        - [3.3.3 降级为单体比对（若两复合物均只有 1 条链）](#3-3-3-降级为单体比对-若两复合物均只有-1-条链)
        - [3.3.4 全对全链间单体比对（构建相似度矩阵）](#3-3-4-全对全链间单体比对-构建相似度矩阵)
        - [3.3.5 初始链配对（enhanced_greedy_search）](#3-3-5-初始链配对-enhanced_greedy_search)
        - [3.3.6 二聚体特殊处理与寡聚体标志设定](#3-3-6-二聚体特殊处理与寡聚体标志设定)
        - [3.3.7 质心优化（若配对≥3 或 is_oligomer，且无用户映射、非 -se）](#3-3-7-质心优化-若配对-3-或-is_oligomer-且无用户映射-非--se)
        - [3.3.8 备份初始配对数据](#3-3-8-备份初始配对数据)
        - [3.3.9 迭代叠加优化（MMalign_iter）](#3-3-9-迭代叠加优化-mmalign_iter)
        - [3.3.10 特殊快速执行（仅 -byresi 且大型寡聚体）](#3-3-10-特殊快速执行-仅--byresi-且大型寡聚体)
        - [3.3.11 回退检查（若迭代总分低于最佳单体得分）](#3-3-11-回退检查-若迭代总分低于最佳单体得分)
        - [3.3.12 交叉链比对（针对同源二聚体，总残基数 \< 10000）](#3-3-12-交叉链比对-针对同源二聚体-总残基数-10000)
      - [3.4 核心数据流串联图](#3-4-核心数据流串联图)
      - [3.5 代码执行逻辑详细流程图](#3-5-代码执行逻辑详细流程图)
      - [3.6 总结](#3-6-总结)
      - [3.7 函数优化点建议](#3-7-函数优化点建议)
        - [3.7.1 处理用户强制链映射（-chainmap）阶段](#3-7-1-处理用户强制链映射--chainmap-阶段)
        - [3.7.2 内层循环的内存复用](#3-7-2-内层循环的内存复用)
        - [3.7.3二级指针整改](#3-7-3二级指针整改)
        - [3.7.4 修复悬空指针传递(安全性)](#3-7-4-修复悬空指针传递-安全性)
        - [3.7.5 交叉链比对之前的更新判断](#3-7-5-交叉链比对之前的更新判断)
    - [MMdock 函数执行逻辑](#mmdock-函数执行逻辑)
      - [4.1 函数目标](#4-1-函数目标)
      - [4.2输入与输出](#4-2输入与输出)
      - [4.3执行流程主线](#4-3执行流程主线)
        - [4.3.1 参数解析(parse_chain_list)](#4-3-1-参数解析-parse_chain_list)
        - [4.3.2 降级为单体比对（若两方均只有一条链）](#4-3-2-降级为单体比对-若两方均只有一条链)
        - [4.3.3 模板剪枝（trimComplex）](#4-3-3-模板剪枝-trimcomplex)
        - [4.3.4 全对全单体比对（构建相似度矩阵）](#4-3-4-全对全单体比对-构建相似度矩阵)
        - [4.3.5 查询链和模板链之间建立一对一初始配对(](#4-3-5-查询链和模板链之间建立一对一初始配对)
        - [enhanced_greedy_search)](#enhanced_greedy_search)
        - [4.3.6 最终优化与输出](#4-3-6-最终优化与输出)
        - [4.3.7 汇总与文件输出](#4-3-7-汇总与文件输出)
      - [4.4 核心数据流串联图](#4-4-核心数据流串联图)
      - [4.5 代码执行逻辑详细流程图](#4-5-代码执行逻辑详细流程图)
      - [4.6 总结](#4-6-总结)
      - [4.7 函数优化点建议](#4-7-函数优化点建议)
        - [4.7.1 全对全循环中的内存复用](#4-7-1-全对全循环中的内存复用)
        - [4.7.2 二级指针整改](#4-7-2-二级指针整改)
    - [mTMalign 函数执行逻辑](#mtmalign-函数执行逻辑)
      - [5.1 函数目标](#5-1-函数目标)
      - [5.2 输入与输出](#5-2-输入与输出)
      - [5.3执行流程主线](#5-3执行流程主线)
        - [5.3.1 解析与预处理](#5-3-1-解析与预处理)
        - [5.3.2 全对全结构比对](#5-3-2-全对全结构比对)
        - [5.3.3 迭代优化](#5-3-3-迭代优化)
          - [5.3.3.1 选取代表链](#5-3-3-1-选取代表链)
          - [5.3.3.2 渐进叠加](#5-3-3-2-渐进叠加)
          - [5.3.3.3 多序列比对构建](#5-3-3-3-多序列比对构建)
          - [5.3.3.4 重新生成对比对序列](#5-3-3-4-重新生成对比对序列)
          - [5.3.3.5 重评分与收敛判断](#5-3-3-5-重评分与收敛判断)
        - [5.3.4 最终统计计算](#5-3-4-最终统计计算)
        - [5.3.5 结果输出](#5-3-5-结果输出)
        - [5.3.6 可选输出](#5-3-6-可选输出)
        - [5.3.7 内存清理](#5-3-7-内存清理)
      - [5.4 核心数据流闭环](#5-4-核心数据流闭环)
      - [5.5 代码执行逻辑详细流程图](#5-5-代码执行逻辑详细流程图)
      - [5.6总结](#5-6总结)
      - [5.7 函数优化点建议](#5-7-函数优化点建议)
        - [5.7.1 mTMalign算法功能完善](#5-7-1-mtmalign算法功能完善)
        - [5.7.2 全对全循环中的内存复用](#5-7-2-全对全循环中的内存复用)
        - [5.7.3 迭代叠加部分内存的重复分配](#5-7-3-迭代叠加部分内存的重复分配)
        - [5.7.4 二级指针整改](#5-7-4-二级指针整改)
    - [SOIalign函数执行逻辑](#soialign函数执行逻辑)
      - [6.1 函数目标](#6-1-函数目标)
      - [6.2 输入与输出](#6-2-输入与输出)
      - [6.3 执行流程主线](#6-3-执行流程主线)
        - [6.3.1 文件解析](#6-3-1-文件解析)
        - [6.3.2 计算链的二级结构](#6-3-2-计算链的二级结构)
        - [6.3.3 为每个残基构造局部最近邻上下文](#6-3-3-为每个残基构造局部最近邻上下文)
        - [6.3.4 结构比对过程](#6-3-4-结构比对过程)
        - [6.3.5 结果输出与内存清理](#6-3-5-结果输出与内存清理)
      - [6.4 核心数据流闭环](#6-4-核心数据流闭环)
      - [6.5代码执行逻辑详细流程图](#6-5代码执行逻辑详细流程图)
      - [6.6 总结](#6-6-总结)
      - [6.7 函数优化点建议](#6-7-函数优化点建议)
        - [6.7.1 内层循环的内存复用](#6-7-1-内层循环的内存复用)
        - [6.7.2 序列顺序无关比对专用数组的重复分配](#6-7-2-序列顺序无关比对专用数组的重复分配)
        - [6.7.3 二级结构边界数组的重复分配](#6-7-3-二级结构边界数组的重复分配)
        - [6.7.4 临时数组 invmap 和 dist_list 的分配](#6-7-4-临时数组-invmap-和-dist_list-的分配)
    - [flexalign 函数执行逻辑](#flexalign-函数执行逻辑)
      - [7.1 函数目标](#7-1-函数目标)
      - [7.2 输入与输出](#7-2-输入与输出)
      - [7.3 执行流程主线](#7-3-执行流程主线)
        - [7.3.1 文件解析](#7-3-1-文件解析)
        - [7.3.2 柔性结构比对](#7-3-2-柔性结构比对)
        - [7.3.3 自适应回退机制](#7-3-3-自适应回退机制)
        - [7.3.4 结果输出](#7-3-4-结果输出)
        - [7.3.5 内存清理](#7-3-5-内存清理)
      - [7.4 核心数据流闭环](#7-4-核心数据流闭环)
      - [7.5代码执行逻辑详细流程图](#7-5代码执行逻辑详细流程图)
      - [7.6 总结](#7-6-总结)
      - [7.7 函数优化点建议](#7-7-函数优化点建议)
        - [7.7.1 内层循环的内存复用](#7-7-1-内层循环的内存复用)
  - [第二部分 各比对流程算法分析](#第二部分-各比对流程算法分析)
    - [CPalign_main 函数执行逻辑](#cpalign_main-函数执行逻辑)
      - [8.1 函数整体功能](#8-1-函数整体功能)
      - [8.2 输入与输出](#8-2-输入与输出)
      - [8.3 函数的执行流程](#8-3-函数的执行流程)
        - [8.3.1 复制并拼接双倍结构](#8-3-1-复制并拼接双倍结构)
        - [8.3.2 拼接双倍结构与目标结构进行一次快速比对](#8-3-2-拼接双倍结构与目标结构进行一次快速比对)
        - [8.3.3 压缩比对序列](#8-3-3-压缩比对序列)
        - [8.3.4 滑动窗口寻找最优断点](#8-3-4-滑动窗口寻找最优断点)
        - [8.3.5 原始结构与目标结构进行一次快速比对](#8-3-5-原始结构与目标结构进行一次快速比对)
        - [8.3.6 决定是否采纳环化排列结构策略](#8-3-6-决定是否采纳环化排列结构策略)
        - [8.3.7 根据断点重排结构](#8-3-7-根据断点重排结构)
        - [8.3.8 用重排后的结构再进行一次快速比对](#8-3-8-用重排后的结构再进行一次快速比对)
        - [8.3.9 最终精确比对](#8-3-9-最终精确比对)
        - [8.3.10 生成比对序列的输出格式](#8-3-10-生成比对序列的输出格式)
        - [8.3.11 清理内存](#8-3-11-清理内存)
      - [8.4 代码执行逻辑详细流程图](#8-4-代码执行逻辑详细流程图)
      - [8.5 总结](#8-5-总结)
      - [8.6 函数优化点建议](#8-6-函数优化点建议)
        - [8.6.1 滑动窗口寻找最佳断点算法优化](#8-6-1-滑动窗口寻找最佳断点算法优化)
    - [TMalign_main 函数执行逻辑](#tmalign_main-函数执行逻辑)
      - [9.1 函数整体功能](#9-1-函数整体功能)
      - [9.2 输入与输出](#9-2-输入与输出)
      - [9.3 函数的执行流程](#9-3-函数的执行流程)
        - [9.3.1 内存分配与参数初始化](#9-3-1-内存分配与参数初始化)
        - [9.3.2 处理用户指定对齐（-I 模式：i_opt==3）](#9-3-2-处理用户指定对齐--i-模式-i_opt-3)
        - [9.3.3 生成多策略初始对齐映射（仅 i_opt ≤ 1 时）](#9-3-3-生成多策略初始对齐映射-仅-i_opt-1-时)
        - [9.3.4 处理用户初始对齐（-i 模式：i_opt == 1 或 2）](#9-3-4-处理用户初始对齐--i-模式-i_opt-1-或-2)
        - [9.3.5 对齐有效性检查](#9-3-5-对齐有效性检查)
        - [9.3.6精细搜索确定最终旋转矩阵](#9-3-6精细搜索确定最终旋转矩阵)
        - [9.3.7 选高质量对齐映射残基对](#9-3-7-选高质量对齐映射残基对)
        - [9.3.8 计算多种归一化下的最终 TM-score](#9-3-8-计算多种归一化下的最终-tm-score)
        - [9.3.9 生成格式化的比对序列](#9-3-9-生成格式化的比对序列)
        - [9.3.10 释放内存并返回](#9-3-10-释放内存并返回)
      - [9.4 代码执行逻辑详细流程图](#9-4-代码执行逻辑详细流程图)
      - [9.5 总结](#9-5-总结)
      - [9.6 函数优化点建议](#9-6-函数优化点建议)
        - [9.6.1 在5 种不同的启发式初始对齐策略中引入提前结束机制](#9-6-1-在5-种不同的启发式初始对齐策略中引入提前结束机制)
        - [9.6.2 动态规划与内存布局优化](#9-6-2-动态规划与内存布局优化)
        - [9.6.3 计算最终 TM-score(TM1~TM5)存在大量重复的局部叠加搜索](#9-6-3-计算最终-tm-score-tm1-tm5-存在大量重复的局部叠加搜索)
        - [9.6.4 get_initial5内部4层for循环中，存在多余的Kabsch 计算](#9-6-4-get_initial5内部4层for循环中-存在多余的kabsch-计算)
<!-- /TOC -->
...
分段对齐序列（不同铰链段用 '0'、'1'、'2'…标记，空格 ' ' 表示未对齐）
