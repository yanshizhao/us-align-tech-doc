[第一部分 C语言语法风格梳理
[2](#第一部分-c语言语法风格梳理)](#第一部分-c语言语法风格梳理)

[1.1比对主入口函数（USalign.cpp）
[2](#比对主入口函数usalign.cpp)](#比对主入口函数usalign.cpp)

[1.2 核心算法实现函数 [2](#核心算法实现函数)](#核心算法实现函数)

[1.2.1坐标变换与叠加 [2](#坐标变换与叠加)](#坐标变换与叠加)

[1.2.2 序列比对 [3](#序列比对)](#序列比对)

[1.2.3 结构比对核心 [3](#结构比对核心)](#结构比对核心)

[1.3 IO与工具函数 [3](#io与工具函数)](#io与工具函数)

[1.3.1内存管理 [3](#内存管理)](#内存管理)

[1.3.2 文件读取与解析 [4](#文件读取与解析)](#文件读取与解析)

[1.3.3 结果输出 [4](#结果输出)](#结果输出)

[1.3.4 通用工具 [4](#通用工具)](#通用工具)

[第二部分 C风格代码整改为C++风格影响分析(重点关注性能)
[5](#第二部分-c风格代码整改为c风格影响分析重点关注性能)](#第二部分-c风格代码整改为c风格影响分析重点关注性能)

[2.1 正面性能提升 [5](#正面性能提升)](#正面性能提升)

[2.2 中性性能影响 [5](#中性性能影响)](#中性性能影响)

[2.3 负面性能影响 [5](#负面性能影响)](#负面性能影响)

[第三部分 US-align 项目基础层优化（不修改算法）
[6](#第三部分-us-align-项目基础层优化不修改算法)](#第三部分-us-align-项目基础层优化不修改算法)

[3.1 批量处理无多线程并行
[6](#批量处理无多线程并行)](#批量处理无多线程并行)

[3.2 核心计算循环向量化 [6](#核心计算循环向量化)](#核心计算循环向量化)

[3.3 Kabsch 算法自实现未优化
[7](#kabsch-算法自实现未优化)](#kabsch-算法自实现未优化)

[3.4 序列比对动态规划缓存效率低
[8](#序列比对动态规划缓存效率低)](#序列比对动态规划缓存效率低)

[3.5 IO与PDB解析冗余开销
[8](#io与pdb解析冗余开销)](#io与pdb解析冗余开销)

[3.6 性能优化总结 [9](#性能优化总结)](#性能优化总结)

[第四部分 US-align编码规范优化
[9](#第四部分-us-align编码规范优化)](#第四部分-us-align编码规范优化)

[4.1 可读性优化 [9](#可读性优化)](#可读性优化)

[4.2 可维护性优化 [10](#可维护性优化)](#可维护性优化)

[4.3 用结构体封装参数，消除超长参数列表
[11](#用结构体封装参数消除超长参数列表)](#用结构体封装参数消除超长参数列表)

[4.4 用RAII(Resource Acquisition Is Initialization,
资源获取即初始化）容器代替手动内存管理，消除内存泄漏
[12](#用raiiresource-acquisition-is-initialization-资源获取即初始化容器代替手动内存管理消除内存泄漏)](#用raiiresource-acquisition-is-initialization-资源获取即初始化容器代替手动内存管理消除内存泄漏)

[4.5 补充单元测试，保证修改不破坏原有逻辑
[12](#补充单元测试保证修改不破坏原有逻辑)](#补充单元测试保证修改不破坏原有逻辑)

[4.6 健壮性优化 [12](#健壮性优化)](#健壮性优化)

[第五部分 后续工作计划 [13](#后续工作计划)](#后续工作计划)

[5.1 梳理us-align 项目源码架构(\~2周左右)
[13](#梳理us-align-项目源码架构2周左右)](#梳理us-align-项目源码架构2周左右)

[5.2
针对项目补充单元测试用例，为后续重构和优化结果兜底（待定）。*动代码先完成*
[14](#针对项目补充单元测试用例为后续重构和优化结果兜底待定动代码先完成)](#针对项目补充单元测试用例为后续重构和优化结果兜底待定动代码先完成)
...
假设 A 是 float（占4字节），Narray2 是 4。当执行 new A \[4\]
时，内存是这样的：
